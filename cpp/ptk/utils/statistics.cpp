...
std::chrono::time_point<std::chrono::system_clock> Statistics::time_last_called_ =
    std::chrono::system_clock::now();
double Statistics::print_frequency = 20.0; //minimum print frequency


Statistics::Statistics() : max_tag_length_(15) {}
...
StatsCollectorImpl::StatsCollectorImpl(size_t handle) : handle_(handle),count(0) {}
...
void StatsCollectorImpl::IncrementCount() {
  Statistics::Instance().AddSample(handle_, count);
  count++;
}

void Statistics::SetPrintFrequency(double freq) {
...
void Statistics::Print(std::ostream& out) {  // NOLINT
  ...
  if (tag_map.empty()) {
    ...
    double dt = static_cast<double>(
                    std::chrono::duration_cast<std::chrono::nanoseconds>(
                        ...
                        .count()) *kNumSecondsPerNanosecond;
    double f = 1.0/dt;

    if(f < print_frequency) {
  ...
  out << "Statistics (Running at " << print_frequency << " Hz)\n"; 
...
void Statistics::WriteAllSamplesToCsvFile(const std::string& path) {
  ...
  if (!output_file) {
    // ORBV_ERROR_MSG("Could not write samples: Unable to open file: " << path);
    return;
  }

//   ORBV_INFO_MSG("Writing statistics to file: " << path);
...
void Statistics::WriteToYamlFile(const std::string& path) {
  ...
  if (!output_file) {
    // ORBV_ERROR_MSG("Could not write statistics: Unable to open file: " << path);
    return;
  }
...
//   ORBV_ERROR_MSG("Writing statistics to file: " << path);
  for (const map_t::value_type& tag : tag_map) {
    const size_t index = tag.second;
