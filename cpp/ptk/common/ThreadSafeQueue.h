...
//taken with inspiration from https://github.com/MIT-SPARK/Kimera-VIO/blob/master/include/kimera-vio/utils/ThreadsafeQueue.h
...
class ThreadsafeQueue : public ThreadsafeQueueBase<T> {
 	public:
		...
		explicit ThreadsafeQueue(int queue_size_ = -1);
...
template <typename T>
ThreadsafeQueue<T>::ThreadsafeQueue(int queue_size_)
